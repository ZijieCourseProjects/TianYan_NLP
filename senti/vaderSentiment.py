# coding: utf-8
# Author: C.J. Hutto
# Adapted to TianYan platform by Colin Z
# MIT License
"""
...
def make_booster(custom_booster, custom_damper):
    global BOOSTER_DICT

    if custom_booster is not None:
        for word in custom_booster:
            BOOSTER_DICT[word] = B_INCR
    else:
        BOOSTER_DICT.update(INCR_BOOSTER)

    if custom_damper is not None:
        for word in custom_damper:
            BOOSTER_DICT[word] = B_DECR
    else:
        BOOSTER_DICT.update(DECR_BOOSTER)


def make_negation_list(custom_negation):
    global NEGATE
    if custom_negation is not None:
        NEGATE.extend(custom_negation)
    else:
        NEGATE.extend(DEFAULT_NEGATE)
...
class SentimentIntensityAnalyzer(object):
    ...
    def __init__(self, custom_lexicon=None, custom_booster=None, custom_damper=None, custom_negation=None):
        _this_module_file_path_ = os.path.abspath(getsourcefile(lambda: 0))
        if custom_lexicon is not None:
            self.lexicon = custom_lexicon
        else:
            lexicon_full_filepath = os.path.join(os.path.dirname(_this_module_file_path_), "vader_lexicon.txt")
            with codecs.open(lexicon_full_filepath, encoding='utf-8') as f:
                self.lexicon_full_filepath = f.read()
            self.lexicon = self.make_lex_dict()

        emoji_full_filepath = os.path.join(os.path.dirname(_this_module_file_path_), "emoji_utf8_lexicon.txt")
        with codecs.open(emoji_full_filepath, encoding='utf-8') as f:
            self.emoji_full_filepath = f.read()
        self.emojis = self.make_emoji_dict()

        make_booster(custom_booster, custom_damper)
        make_negation_list(custom_negation)
    ...
    def sentiment_valence(self, valence, sentitext, item, i, sentiments):
        ...
        if item_lowercase in self.lexicon:
            ...
            if item_lowercase == "no" and i != len(words_and_emoticons) - 1 and words_and_emoticons[i + 1].lower() in self.lexicon:
                # don't use valence of "no" as a lexicon item. Instead, set its valence to 0.0 and negate the next item
                valence = 0.0
